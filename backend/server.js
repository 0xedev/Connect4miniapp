...
app.get("/stats", (req, res) => {
  const roomStats = {
    ...
    activeGames: [...rooms.values()].filter((r) => r.gameState === "playing")
      .length,
    totalPlayers: [...rooms.values()].reduce(
      (sum, r) => sum + r.players.length,
      0
    ),
    totalSpectators: [...rooms.values()].reduce(
      (sum, r) => sum + r.spectators.length,
      0
    ),
  };
